...
def diary_main():
    ...
    root= tk.Tk()
    # '제목'이라는 단어를 입력시키기
    title = lb.label(main.main_frame, "제목", 0, 0, "ne")
    title.lb_meker()
    # 제목을  쓸 수 있는 입력칸 생성
    title_txt = tx.text(main.main_frame, 100, 1, 0, 1, "ne")
    title_txt.txt_maker()
    # '내용'이라는 단어를 입력시키기
    content = lb.label(main.main_frame, "내용", 1, 0, "ne")
    content.lb_meker()
    # 내용을 쓸 수 있는 입력칸 생성
    content_txt = tx.text(main.main_frame, 100, 50, 1, 1, "ne")
    content_txt.txt_maker()
    # 저장 버튼 생성
    save = bt.button(main.main_frame, "저장", 2, 2, "n", msg_save)
    save.bnt_maker()
